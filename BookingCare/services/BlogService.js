...
const getAllBlogs = async (params = {}) => {
  ...
  try {
    ...
    try {
      ...
      // Đảm bảo chỉ trả về tối đa 3 bài viết nếu có limit=3
      if (params.limit === 3 && Array.isArray(response.data)) {
      ...
      if (
        ...
        try {
          ...
          if (newToken) {
            ...
            console.log(
              "Kết quả API bài viết blog (sau khi refresh):",
              retryResponse.data
            );
            // Đảm bảo chỉ trả về tối đa 3 bài viết nếu có limit=3
            if (params.limit === 3 && Array.isArray(retryResponse.data)) {
              return retryResponse.data.slice(0, 3);
            }
      ...
      // Đảm bảo chỉ trả về tối đa 3 bài viết nếu có limit=3
      return params.limit === 3 ? sampleBlogs.slice(0, 3) : sampleBlogs;
    }
  } catch (error) {
  ...
  return [
    {
      ...
      content:
        "<p>Nội dung bài viết về chăm sóc da. Da đẹp không chỉ đến từ gen tốt mà còn phụ thuộc nhiều vào cách bạn chăm sóc hàng ngày...</p>",
    ...
    },
    {
      ...
      content:
        "<p>Trong thời đại hiện nay, việc chăm sóc bản thân không chỉ là nhu cầu mà còn là cách để cân bằng cuộc sống. Các dịch vụ spa cao cấp đang trở thành lựa chọn hàng đầu của nhiều người...</p>",
      excerpt:
        "Những dịch vụ spa được khách hàng yêu thích nhất tại Booking Care",
    ...
    },
    {
      ...
      content:
        "<p>Làm đẹp không nhất thiết phải tốn nhiều tiền. Có rất nhiều nguyên liệu tự nhiên trong nhà bếp có thể giúp bạn chăm sóc da và tóc hiệu quả...</p>",
...
const getBlogsByCategory = async (categoryId) => {
  ...
  try {
    ...
    const response = await axiosJWT.get(
      `/api/v1/blog-categories/${categoryId}/blogs`,
      {
        headers: {
          "Content-Type": "application/json",
          ...(token && { Authorization: `Bearer ${token}` }),
...
const uploadBlogImage = async (file) => {
  ...
  try {
    ...
    throw new Error(
      `Không thể phân tích URL ảnh từ response: ${JSON.stringify(
        response.data
      )}`
    );
  } catch (error) {
