  ...
  async (error) => {
    ...
    if (!error.response || !originalRequest) {
      ...
      if (!error.response && !originalRequest._retryCount) {
        ...
        console.log(
          `Thử lại request sau ${retryDelay}ms (lần ${originalRequest._retryCount}/${MAX_RETRY_COUNT})`
        );
    ...
    if (error.response.status === 401 && !originalRequest._retry) {
      ...
      if (isRefreshing) {
        console.log(
          "Đang có refresh token khác đang xử lý, request hiện tại sẽ đợi"
        );
      ...
      try {
        ...
        if (response.data && response.data.token) {
          ...
          console.log(
            "Không thể refresh token, chuyển hướng đến trang đăng nhập"
          );
      ...
      switch (status) {
        ...
        case 504:
          ...
          if (originalRequest._serverErrorRetryCount <= 2) {
            ...
            console.log(
              `Thử lại request sau lỗi máy chủ sau ${retryDelay}ms (lần ${originalRequest._serverErrorRetryCount}/2)`
            );
...
const AdminService = {
  ...
  getRecentBookings: async (limit = 5) => {
    try {
      const response = await axiosInstance.get(
        `/api/v1/admin/recent-bookings?limit=${limit}`,
        {
          headers: getAuthHeader(),
        }
      );
  ...
  getPopularServices: async (limit = 5) => {
    try {
      const response = await axiosInstance.get(
        `/api/v1/admin/popular-services?limit=${limit}`,
        {
          headers: getAuthHeader(),
        }
      );
    ...
    try {
      ...
      if (error.response) {
        ...
        switch (error.response.status) {
          ...
          case 403:
            console.error(
              "Lỗi phân quyền: Không có quyền xem danh sách người dùng"
            );
            // Thông báo cho người dùng
            alert(
              "Bạn không có quyền xem danh sách người dùng. Vui lòng liên hệ quản trị viên."
            );
            break;
          ...
          default:
            console.error(
              `Lỗi HTTP ${error.response.status}:`,
              error.response.data
            );
        }
  ...
  getUserPermissionGroups: async (userId) => {
    try {
      const response = await axiosInstance.get(
        `/api/v1/permissions/users/${userId}/groups`,
        {
          headers: getAuthHeader(),
        }
      );
  ...
  getAllUserPermissionGroups: async () => {
    try {
      const response = await axiosInstance.get(
        `/api/v1/permissions/users/all-permissions`,
        {
          headers: getAuthHeader(),
        }
      );
  ...
  createPermissionGroup: async (groupData) => {
    try {
      ...
      console.log("Creating permission group:", {
        ...
        permissionCount: groupData.permissions
  ...
  assignPermissionGroupToUser: async (userId, groupId) => {
    try {
      await axiosInstance.post(
        `/api/v1/permissions/users/${userId}/groups/${groupId}`,
        {},
        {
          headers: getAuthHeader(),
        }
